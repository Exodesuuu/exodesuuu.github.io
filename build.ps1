...
if ((Test-Path $indexSrcPath) -and (Test-Path $appSrcPath)) {
    # 1. Read files
    $indexHtml = Get-Content $indexSrcPath -Raw
    $appJs = Get-Content $appSrcPath -Raw
    ...
    if ($startIndex -ge 0 -and $endIndex -ge 0) {
        ...
        # 3. Combine JS code with HTML injection (in-memory)
        $htmlInjection = "document.addEventListener('DOMContentLoaded', () => {`r`n  document.body.innerHTML = decodeURIComponent(escape(window.atob('$bodyBase64')));"
        ...
        $obfuscatedJs = "eval(decodeURIComponent(escape(window.atob('$finalBase64'))));"
        ...
        # Note: We keep the <body> tag classes and script tags, but clear the visual contents
        ...
        Write-Host "HTML and JS compiled, combined, and scrambled successfully."
    } else {
